...
export default async function FloorProgressPage({
  ...
  const { data: projectRows, error: projectRowsError } = await supabase
    ...
    .eq('status', 'open')
  if (projectRowsError) console.error('FloorProgressPage: projects read failed', projectRowsError)
  ...
  const { data: memberRows, error: memberRowsError } = await supabase
    ...
    .eq('is_active', true)
  if (memberRowsError) console.error('FloorProgressPage: members read failed', memberRowsError)
  ...
  return (
    <>
      ...
      <CrossProjectList
        titleKey="navExecFloorProgress"
        scope={scope}
        basePath="/floor-progress"
