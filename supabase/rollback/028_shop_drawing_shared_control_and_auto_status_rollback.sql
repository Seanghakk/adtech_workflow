...
-- before the functions they call, then the new drafting_started_at
-- column, then restores both policy pairs to their pre-028 text
-- (migration 019's shape for shop_drawing_items_update; migration 027's
-- shape for shop_drawing_submissions_insert/update) — RE-CREATING the
-- old policy text rather than merely dropping it, so the table is never
-- left with NO update/insert policy at all in between (drop-then-recreate
-- happens inside one transaction, so this is atomic either way, but
-- stating the old text explicitly here means this rollback does not
-- depend on any OTHER migration file still being readable at rollback
-- time).
--
...
--
-- DATA LOSS, INHERENT NOT A DEFECT (updated for this brief's own
-- drafting_started_at addition): migration 028 added ONE new column,
-- drafting_started_at — dropping it loses whatever it recorded, same as
-- every other rollback file in this project only ever undoes schema, not
-- history. Every status/updated_at value migration 028's triggers wrote
-- while applied stays exactly as it is — rolling back the MECHANISM does
-- not undo values already written to columns that still exist afterward.
-- =============================================================================
...
alter table workflow.shop_drawing_items
  drop column if exists drafting_started_at;
