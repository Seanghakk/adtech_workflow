...
-- 1. All six tables exist with the right column shapes.
...
-- tender_boq_location_map.floor_id nullable (TRUE), every other FK/quantity
-- column NOT nullable (FALSE) except the freeform Contract BOQ fields
-- (section_label, brand) and the optional brand/model/part_number/remarks
-- columns on Tender/Shop Drawing BOQ lines.
...
where table_schema = 'workflow'
  and table_name in (
    ...
    'contract_boq_lines',
    'shop_drawing_boq_lines', 'shop_drawing_boq_line_locations'
  )
...
--    and shop_drawing_boq_line_locations.
...
-- 4. RLS is actually ENABLED on all six tables, not just carrying
...
--    relrowsecurity = true on all six rows.
...
--    anywhere yet. Expect 6 rows total, all cmd = 'r' (select).
-- The three project-scoped policies (tender_boq_lines_select,
-- contract_boq_lines_select, shop_drawing_boq_lines_select,
-- tender_boq_location_map_select) should each mention BOTH is_member()
-- and can_view_project in their qual text. The two child-table policies
-- (tender_boq_line_locations_select, shop_drawing_boq_line_locations_select)
-- should each mention is_member() AND an exists-join through their own
-- parent table's name — if either mentions only one of the two, the
-- Brief 022 Amendment A/B pattern this migration copied was not applied
-- correctly.
...
--    system_type, part_number) — the most reliable shared key available
--    without a dedicated cross-tier item code (see the migration's own
--    §1 comment on why one was not invented).
select
  t.project_id,
  t.system_type,
  t.part_number,
...
--      confirm all six tables SELECT normally for that project's rows.
--   b. As a sales-restricted member who canNOT view a given project,
--      confirm all six tables return zero rows for it, same as
...
--      six tables through the app's own anon/authenticated credentials —
