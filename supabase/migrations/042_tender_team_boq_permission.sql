-- Brief 104 — the Tender team may write its own BOQ tier.
--
-- THE FINDING, from Brief 099's own result doc. Each BOQ tier's write
-- rule belongs to whoever owns that tier:
--
--     contract_boq_lines       the project's PIC, or superadmin
--     shop_drawing_boq_lines   superadmin, or current_team() in
--                              ('shop_drawing','a_and_a')
--     tender_boq_lines         SUPERADMIN ONLY
--
-- The Tender team exists — confirmed in workflow.teams as code 'tender',
-- label 'Tender', on both production and rollback-test, and ONE
-- production member holds it today — and it is who prepares a tender.
-- But no rule anywhere let it write the tender BOQ, so only a superadmin
-- could import or edit one.
--
-- That is the fourth and last known instance of one pattern: the team
-- that does the work absent from the policy that lets them start it.
-- The others were A&A missing from the AutoCAD export (038), and Shop
-- Drawing and A&A unable to CREATE a drawing they could already draft,
-- check and submit (039).
--
-- DECIDED 25 Sep 2026: the Tender team may write its own tier.
--
-- WIDENED, NEVER REPLACED. Each policy keeps superadmin exactly as it
-- had it; the team clause is added. SELECT is untouched — it was
-- is_member() + can_view_project and stays that way, because everyone
-- who can see the project could already read the tender BOQ and this
-- changes nothing about that.
--
-- The team clause is written the same way shop_drawing_boq_lines writes
-- its own, so the tiers read alike.
...
create policy tender_boq_lines_insert on workflow.tender_boq_lines
  ...
  with check (
    ...
    or workflow.current_team() = any (array['tender'::text])
  );
...
comment on policy tender_boq_lines_insert on workflow.tender_boq_lines is
  'Brief 104 — superadmin or the Tender team. Written to match shop_drawing_boq_lines_insert''s shape so the tiers read alike.';

-- ------------------------------------------------- commit_boq_import
-- The import routes through this SECURITY DEFINER function, which
-- carries its own transcription of each tier's rule (migration 037, as
-- corrected by Brief 099). Widening the policies alone would leave the
-- import still refusing a Tender member.
--
-- The body below is production's CURRENT definition with exactly TWO
-- lines changed — the tender branch of v_may_write_tier, and the tender
-- branch of the refusal message. Everything else is byte-identical to
-- what is live, captured with pg_get_functiondef rather than retyped, so
-- nothing else can drift in.
